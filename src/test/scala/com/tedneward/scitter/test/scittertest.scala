package com.tedneward.scitter.test {
  class ScitterTests {
    ...
    // set twitter credencials for test purpose...
    def testUser = ""
    def testPassword = ""
  ...
  class ExplorationTests {
    ...
    @Test def callTwitterTest = {
      val testURL = "https://twitter.com/hashtag/jn?src=rela"
            ...
            contents.foreach((c) =>
              c match {
                ...
                case <user>{ contents2 @ _* }</user> => {
                  contents2.foreach((c2) =>
                    c2 match {
                      case <screen_name>{ u }</screen_name> => System.out.println("\tUser: " + u.text.trim)
                      case _ => ()
                    })
                }
                case _ => ()
